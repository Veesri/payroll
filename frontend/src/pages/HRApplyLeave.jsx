...
const HRApplyLeave = () => {
    ...
    const fetchLeaves = async () => {
        try {
            // Re-using the same GET endpoint, it will auto-filter via backend role checks
            const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/leaves/?page=${page}`, { headers });
            // HR sees all requests waiting for Super Admin AND their own if we modified the query correctly...
            // Wait, the backend currently returns ALL leaves where approval_level='hr_admin' for HR.
            // Oh! If the HR is viewing this page, we actually want them to see their OWN leaves.
            // But the backend `get_leaves` currently forces `elif user.role == 'hr_admin': query = query.filter_by(approval_level='hr_admin')`
            // Let's pass a special query param `?personal=true`
            const personalRes = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/leaves/?page=${page}&personal=true`, { headers });
            setLeaves(personalRes.data.leaves);
            setTotalPages(personalRes.data.pages);
        } catch (error) {
    ...
    const handleApplyLeave = async (e) => {
        ...
        try {
            ...
            alert("Leave application submitted successfully. Escalated to Super Admin.");
            setShowModal(false);
        ...
        <Layout>
            <div className="slide-up-animation">
                <div className="mb-5 d-flex justify-content-between align-items-center">
                    <div>
                        <h1 className="fw-bold mb-0">My Leave Application</h1>
                        <p className="text-muted">Apply for leave and view your personal leave history</p>
                    </div>
                        ...
                        <table className="table table-borderless table-hover text-white align-middle">
                            <thead className="text-muted border-bottom border-secondary">
                                <tr>
                                    ...
                                    <th>Applied On</th>
                                    <th>Status</th>
                                    <th>Comments (from Super Admin)</th>
                                </tr>
                            </thead>
                            <tbody>
                                {leaves.map(leave => (
